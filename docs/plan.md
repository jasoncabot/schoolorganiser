...
1. **Scaffold:** Worker and Agents SDK, `wrangler.jsonc` (with an `e2e` environment), `@cloudflare/vitest-plugin` with the AI and email stub Workers, the `Deps` object, ESLint, Prettier, strict `tsc`, Playwright, the Tailwind v4 build, `.dev.vars.example` and `.gitignore`.
